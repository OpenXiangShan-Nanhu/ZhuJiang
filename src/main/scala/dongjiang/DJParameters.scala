...
case class DJParam(
  ...
  require(sfSets  >= nrDirBank.max(2), s"illegal sf sets : sfSets($sfSets)   = sfSizeInB($sfSizeInB)   / CacheLine($CacheLine) / llcWays($llcWays) > max(nrDirBank($nrDirBank), 2)")
...
trait HasDJParam extends HasParseZJParam {
  ...
  lazy val TIMEOUT_DATACM   = 10000 * 4
